...
Most of the code of the nodes which are running is in ```bash /services``` folder with verifier, issuer, centre and wallet-cli. \
The chaincode (smart contract) is in ```bash /chaincode/ssi-contract/index.js``` and its interface is in ```bash /scripts/ff-contract-interface.json```\
Fabric Binaries are in ```bash fabric-samples/bin```\
Also ```bash /crypto``` folder is auto generated by firefly.
